...
public class buscaminas extends JFrame implements ActionListener 
{
	...
	private ButtonGroup nivelesButtonGroup;//puede ser reemplazado por una variable local
	private int puntos=0;
	private int nivel=0; //Evitar el uso de campos privados como "nivel"
	private int minas=10;
	private int arreglo[][]= new int [7][7];
	private Icon error = new ImageIcon(getClass().getResource("error.png")); //puede ser final. Solo es inicializado en la declaracion o constructor
	private Icon carita = new ImageIcon(getClass().getResource("carita.png"));//puede ser final. Solo es inicializado en la declaracion o constructor
	
	public buscaminas()
	{
		super("Juego de Buscaminas");
		initmenu(); //metodo sobrecargado
		initcomponents();//metodo sobrecargado
		insertarelementos();//metodo sobrecargado
		valoresiniciales();//metodo sobrecargado
			...
			if (arreglo[cuenta][cuentadentro]==0)
			{
				...
				int continuar=Integer.parseInt(JOptionPane.showInputDialog("Has Perdido\n1.Continuar\n2.Salir"));
							if (continuar==1)
								...
								System.exit(0); //evitar el uso de sentencias if ... else sin el uso de llaver
			}
			else
			{
	...
	public void numerosceros(int n)
	{
		...
		for (int aleatorio=0; aleatorio<n; aleatorio++)
		{
			...
			if (arreglo[num1][num2]!=0) 
				arreglo[num1][num2]=0;//evitar el uso de sentencias if ... else sin el uso de llaver
			else
				aleatorio--;//evitar el uso de sentencias if ... else sin el uso de llaver
		}
	}
